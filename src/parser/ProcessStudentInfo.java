		...
		for (Object str : list.keySet()) {
			for (Object str1 : list.get(str)) {
				...
				if (str.equals("selenium")) {
					connectToSqlDB.InsertDataFromArrayListToMySql(list1, "Selenium", "mapKey", "mapValue");
				} else {
					connectToSqlDB.InsertDataFromArrayListToMySql(list1, "Qtp", "mapKey", "mapValue");
				}
			}
		}

		//Retrieve Selenium Data from Database

		List<String> numbers = connectToSqlDB.readDataBase("Selenium", "mapKey", "mapValue");
		for (String st : numbers) {
			System.out.println(st);
		}

        //Retrieve Qtp Data from Database

        List<String> numbers1 = connectToSqlDB.readDataBase("Qtp", "mapKey", "mapValue");
